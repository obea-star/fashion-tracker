...
async function initDB() {
  ...
  // Backfill missing prices and fix bad ones in background
  setTimeout(() => backfillPrices(), 3000);
  setTimeout(() => fixBadPrices(), 2000);
}
...
async function fixBadPrices() {
  try {
    ...
    for (const item of list) {
      const p = item.savedPrice || item.currentPrice;
      // Clear prices that are suspiciously high (likely parsing errors like 300.00 -> 30000)
      if (p && p > 5000) {
        console.log('Clearing suspicious price for', item.name, ':', p);
        await updateItem(String(item.id), { savedPrice: null, currentPrice: null, priceHistory: [] });
      }
    }
  } catch (e) { console.error('fixBadPrices error:', e.message); }
}
...
async function backfillPrices() {
  try {
    const list = await loadWatchlist();
    for (const item of list) {
      if (item.savedPrice || !item.productUrl) continue;
      try {
        ...
        if (price && price > 0) {
          ...
          console.log('Backfilled price for', item.name, ':', price);
        }
      } catch (e) {
        console.log('Could not backfill price for', item.name, e.message);
      }
    }
...
function extractPrice(html) {
  ...
  function isValidPrice(p) {
    return !isNaN(p) && p >= MIN_PRICE && p <= MAX_PRICE;
  }
  ...
  if (metaPrice) { const p = parseFloat(metaPrice); if (isValidPrice(p)) return p; }
  const patterns = [
    /"price":\s*"?\$?([\d,]+\.\d{2})"?/,
    /"price":\s*"?\$?([\d,]+\.\d{1})"?/,
    /"price":\s*"?\$?([\d,]+)"?/,
...
app.post('/api/search', async (req, res) => {
  ...
  try {
    ...
    console.log("SerpAPI keys:", Object.keys(lensData).join(", "), "matches:", (lensData.visual_matches||[]).length, "error:", lensData.error);
    ...
    console.log('Sending primaryCandidates:', primaryCandidates.length, 'replicas:', replicas.length);
    console.log('First candidate:', JSON.stringify(primaryCandidates[0]));
...
async function sendEmail(to, subject, html) {
  ...
  const body = JSON.stringify({
    from: 'Fashion Tracker <onboarding@resend.dev>',
    to: [to],
    subject,
    html
  });
...
async function sendAlert(to, items, type) {
  const subject = type === 'price'
    ? `📉 Price drop on ${items.length} item${items.length > 1 ? 's' : ''} you're watching`
    : `✅ Back in stock in your size — ${items.length} item${items.length > 1 ? 's' : ''}`;
    ...
    ${items.map(item => `
      <div style="border:1px solid #eee;border-radius:10px;padding:16px;margin:12px 0;display:flex;gap:16px;align-items:flex-start">
        ...
        <div>
          ...
          <span style="color:#999">${item.retailer || ''}</span><br><br>
          ${type === 'price'
            ? `${item.hitTarget ? `<span style="color:${color};font-weight:500">Hit your target!</span><br>` : ''}
               <span style="color:${color};font-size:20px;font-weight:600">$${item.currentPrice}</span>
               ${item.savedPrice ? `<span style="color:#999;text-decoration:line-through;margin-left:8px">$${item.savedPrice}</span>` : ''}
               ${item.dropPercent ? `<span style="background:${color};color:white;padding:2px 8px;border-radius:4px;font-size:12px;margin-left:8px">-${item.dropPercent}%</span>` : ''}`
            : `<span style="color:${color};font-weight:500">Size ${item.size} is back in stock!</span>`}
          <br><br>
          ${item.productUrl ? `<a href="${item.productUrl}" style="background:${color};color:white;padding:8px 16px;border-radius:6px;text-decoration:none;font-size:13px">Shop now →</a>` : ''}
        </div>
      </div>`).join('')}
...
app.get('/api/monitoring-status', async (req, res) => {
  try {
    const list = await loadWatchlist();
    const status = await Promise.all(list.map(async item => {
      let canMonitorPrice = false;
      let canMonitorSize = false;
      let urlType = 'none';

      if (item.productUrl) {
        const url = item.productUrl.toLowerCase();
        if (url.includes('google.com/search') || url.includes('tbm=shop')) {
          urlType = 'google_search';
        } else {
          urlType = 'product_page';
          canMonitorPrice = true;
          canMonitorSize = !!(item.size && !item.noSize);
        }
      }
      ...
      return {
        ...
        canMonitorSize,
        size: item.size || null,
        savedPrice: item.savedPrice || null,
        currentPrice: item.currentPrice || null,
        lastSizeStatus: item.lastSizeStatus || 'unknown',
...
app.post('/api/test-email', async (req, res) => {
  try {
    await sendEmail(
      ...
      '✅ Your Fashion Tracker email alerts are working!',
      `<div style="font-family:sans-serif;max-width:600px;margin:0 auto">
        <h2>Email alerts confirmed ✅</h2>
        <p>Your Fashion Tracker is set up correctly and will email you when:</p>
        <ul style="line-height:2">
          <li>📉 A price drops on any item you're watching</li>
          <li>✅ Your size comes back in stock</li>
          <li>🎯 An item hits your target price</li>
        </ul>
        <p style="color:#999;font-size:12px">Prices are checked every 6 hours. Urgent drops (30%+) are checked every hour.</p>
      </div>`
    );
